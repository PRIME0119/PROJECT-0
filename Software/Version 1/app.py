...
def get_stats():
    ...
    dt = current_time - last_net_time if current_time - last_net_time > 0 else 1
    
    sent_mbps = ((net_io.bytes_sent - last_net_io.bytes_sent) * 8) / 1_000_000 / dt
    recv_mbps = ((net_io.bytes_recv - last_net_io.bytes_recv) * 8) / 1_000_000 / dt
    ...
    try:
        with open("/sys/class/thermal/thermal_zone0/temp", "r") as f:
            ...
            temp_pct = f"{max(0, min(100, (temp_c / 85.0) * 100)):.1f}%"
    except Exception:
        try:
    ...
    print(f"[STATS DEBUG] CPU: {cpu} | Mem: {mem} | Temp_C: {temp_c if 'temp_c' in locals() else 'N/A'} | Network State: {net_state} | Sent: {sent_mbps:.1f} Mbps | Recv: {recv_mbps:.1f} Mbps", flush=True)
    ...
    return jsonify({
        ...
        'temp_pct': temp_pct,
        'net_state': net_state,
        'net_sent': f'{sent_mbps:.1f} Mbps',
        'net_recv': f'{recv_mbps:.1f} Mbps'
    })
